<?php
    ...
    try {     
        ...
        if ($kindNotification == 1) {
            $stmt = $conn->prepare("SELECT * FROM contact WHERE id_questioner=:idQuestioner AND id_answerer=-1");// Chú ý phải sử dụng prepare statement để chống mã độc :))
            ...
            $stmt = $conn->prepare("SELECT * FROM account, contact WHERE id_questioner=:idQuestioner AND id_answerer<>-1 AND id_answerer=id");// Chú ý phải sử dụng prepare statement để chống mã độc :))
            $stmt->bindParam(':idQuestioner', $idQuestioner);
            $stmt->execute();
            $result = $stmt->fetchAll(); // Dùng fetchAll() để trả về mảng toàn bộ dữ liệu hoặc fetch() để lấy từng hàng        
            $resultJson = json_encode($result); // Convert kết quả sang dạng json
